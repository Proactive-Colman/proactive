...
export class TestService {
  ...
  async processPythonFile(fileContent: string, userId: string): Promise<Test> {
    try {
      ...
      const searchIndex = commands.findIndex((cmd) => cmd.includes('send_keys(Keys.ENTER)'));
      if (searchIndex !== -1) {
        commands.splice(
          searchIndex + 1,
          0,
      ...
      const videoIndex = commands.findIndex((cmd) => cmd.includes('ytd-item-section-renderer'));
      if (videoIndex !== -1) {
        commands[videoIndex] = 'driver.find_element(By.CSS_SELECTOR, "ytd-video-renderer").click()';
      }
